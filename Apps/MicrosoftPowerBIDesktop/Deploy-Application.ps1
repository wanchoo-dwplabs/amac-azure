...
Try {
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		$exeInstallParams = "-quiet -norestart ACCEPT_EULA=1 INSTALLDESKTOPSHORTCUT=0"
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		$AppList = Get-InstalledApplication -Name 'Vim 9.1'
		...
		$InstallPath = Join-Path -Path $env:ProgramFiles -ChildPath "Vim"
        if (Test-Path -Path $InstallPath) {
        	Write-Log -Message "Cleanup Vim Program Files Directory."
