...
Try {
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		$AppList = Get-InstalledApplication -Name 'GitHub Desktop'
        foreach ($App in $AppList) {
        	if ($App.UninstallString) {
        		...
        		if (Test-Path -Path $UninstallPath) {
        			...
        			Execute-Process -Path $UninstallPath -Parameters '--uninstall -s'
        			Start-Sleep -Seconds 5
        		}
        	}
        }
        ## Cleanup Notepad ++ Program Files Directory (If Present)
		$InstallPath = Join-Path -Path $env:ProgramFiles -ChildPath "GitHub Desktop"
